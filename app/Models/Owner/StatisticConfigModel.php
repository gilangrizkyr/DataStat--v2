...
class StatisticConfigModel extends Model
{
    ...
    /**
     * Calculate statistic result using ComputationEngine
     * 
     * @param int $statisticId The statistic configuration ID
     * @return array Result with 'success', 'data', and optionally 'error' keys
     */
    public function calculateStatistic($statisticId)
    {
        try {
            // Get statistic configuration
            $statistic = $this->find($statisticId);
            ...
            if ($statistic['is_active'] != 1) {
                return [
                    'success' => false,
                    'error' => 'Statistik tidak aktif'
                ];
            }

            // Use ComputationEngine to calculate
            $computationEngine = new \App\Libraries\ComputationEngine();
            $result = $computationEngine->calculate($statistic);
            ...
            log_message('error', 'Error calculating statistic ' . $statisticId . ': ' . $e->getMessage());
            return [
                'success' => false,
                'error' => 'Gagal menghitung statistik: ' . $e->getMessage()
            ];
        }
    }
}
